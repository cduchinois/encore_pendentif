...
- **Pendant**: Seeed XIAO ESP32-S3 Sense (built-in PDM mic), heart-shaped translucent 3D-printed case (STL files ready: `encore_body.stl` + `encore_lid.stl`, ≈58 × 54 × 18 mm, pulse-wave engraving on the front, integrated bail loop for the chain, mic holes, USB-C slot to charge without opening).
