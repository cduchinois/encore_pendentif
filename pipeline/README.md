...
4. **iTunes links enrichment** (`apple_url`, `spotify_url`, `preview_url`) is online + throttled (3.1 s/req → ~14 h for all). **Done for the demo subset**: the 28 rows matching `demo_tracks.txt` are enriched (all got apple/artwork/preview URLs) — eyeball the rarities (Spiral Tribe etc.), `itunes_lookup` takes the first hit without an artist check. Full-catalog enrichment stays deferred, not blocking.
