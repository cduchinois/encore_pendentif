...
- 2026-07-23 Expert print review: back shell prints on its outer face with zero supports — its edge round is slope-capped by a 60-deg chamfer near the back cap (front keeps the full tangent round; it prints seam-down where any slope is safe).
- 2026-07-23 Case v3.2 — unified print orientation: BOTH shells print on their outer faces with zero supports (symmetric 60-deg chamfered edges, matching bed finish on the joined halves); the pulse switches from raised relief to a 1.2 mm engraving (bed-side = crispest), copper touch glued flat behind it (1.0 mm wall).
- 2026-07-23 Matching chain: print-in-place closed loop (94 tilted interlocked rings, 640 mm = over-the-head matinee length, clasp-free); pendant threads on via the bail. Wearing scale documented against INSEE anthropometry (F 164 / H 177 cm), pendant sits 33 cm below the chin.
- 2026-07-24 Chain: adopt the third-party 64 cm print-in-place Cuban/curb chain (46 chunky links, supportless, clasp-free) over the slim generated rings — sturdier for a party; bail enlarged to Ø9.8/Ø16 to pass its link wire; third-party STL kept out of git (license), embedded in the local viewer only.
