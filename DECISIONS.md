...
- 2026-07-25 Demo set = Madchester -> free-party-techno arc (demo_tracks.txt), drawn from the whole catalog not just Ultimate. Spiral Tribe "Forward The Revolution" = the Shazam-duel finale (signed, so Encore wins offline). Opener = New Order "Blue Monday" (1983).
- 2026-07-25 Gemma-slot "mystery" track = a real unpublished track of Mathieu's (Dub techno 1.m4a, ~129 BPM), NOT a fake beat and deliberately NOT signed / kept out of indexed folders, so the whole ladder misses and only Gemma describes it. Lives on the key at encore_demo/unreleased/, locally at data/samples/unreleased/.
- 2026-07-25 iTunes links enrichment run on the 28 demo rows (apple/artwork/preview urls); full-catalog enrichment stays deferred (online, ~14 h).
